...
Repository containing calibrated SUMO scenarios, vehicle-level autonomous controllers, and an optional corridor-level LLM hierarchy for microsimulation of mixed traffic.

This library uses the TraCI API to directly command the acceleration behavior of autonomous vehicle agents in microsimulation. The hierarchy layer is optional and sits above the existing vehicle-level controller, so the base simulation can still run unchanged when hierarchy is disabled.
...
## Dependencies
    - Python
    - pip
    - requests (optional, only for live AIML-backed hierarchy queries)
...
### Optional hierarchy layer

The repository includes an optional corridor-level hierarchy that can observe traffic-light states, summarize corridor conditions, and, in `live` mode, apply bounded timing bias on top of the base SPaT plan.

Examples:

    python main.py --scenario roosevelt --enable_hierarchy --llm_mode shadow
    python run_hierarchy.py --scenario roosevelt --llm_mode live --llm_model <model_name>

Behavior:

- `shadow` computes hierarchy snapshots and logs without modifying active signal timing
- `live` allows the hierarchy execution planner to apply bounded timing bias
- if no AIML token/backend is available, the coordinator safely falls back to rule-based intents
